...
const Jobs = () => {
    ...
    const { jobs, loading, error } = fetchJobs('https://rest.arbeitsagentur.de/jobboerse/jobsuche-service/pc/v4/jobs?angebotsart=1&wo=Gaienhofen&umkreis=200&arbeitszeit=ho;mj&pav=false');
    console.log(jobs);
    ...
    <div className="flex  flex-wrap gap-4 justify-between jobs-width">
        {
            jobs && jobs.stellenangebote ? ( jobs.stellenangebote.map((job:string, index:number) => (
                <div key={index} className="border border-gray-200 h-52 w-96 shadow-md p-2 rounded-md flex flex-col gap-2">
                    ...
                    <Link to='https://www.arbeitsagentur.de/' target="_blank" 
